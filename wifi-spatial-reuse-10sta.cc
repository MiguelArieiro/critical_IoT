...
using namespace ns3;
uint32_t sta_num = 10;       //number os stations
std::vector<uint32_t> bytesReceived(sta_num+2);

// Global variables for use in callbacks.
std::vector<double> g_signalDbmAvg(sta_num+2);
std::vector<double> g_noiseDbmAvg(sta_num+2);
std::vector<uint32_t> g_samples(sta_num+2);
...
int main(int argc, char *argv[])
{
  ...
  LogComponentEnable("WifiNetDevice", LOG_PREFIX_NODE);

  //criar containers de access points e STA
  NodeContainer wifiStaNodes;
  wifiStaNodes.Create(sta_num);
  ...
  //SSID A definition
  Ssid ssidA = Ssid("A"); //The IEEE 802.11 SSID Information Element.
  mac.SetType("ns3::StaWifiMac",
              "Ssid", SsidValue(ssidA));

  //creating STA A net devices container
  NetDeviceContainer staDeviceA = NetDeviceContainer();
  for (uint32_t i = 0; i < sta_num / 2; i++)
  {
  ...
  Ptr<ApWifiMac> apWifiMac = ap1Device->GetMac()->GetObject<ApWifiMac>();
  ...
  NetDeviceContainer staDeviceB = NetDeviceContainer();
  for (uint32_t i = sta_num / 2; i < sta_num; i++)
  {
    staDeviceB.Add(wifi.Install(spectrumPhy, mac, wifiStaNodes.Get(i)));
  }
  ...
  {
    double throughput = static_cast<double>(bytesReceived[sta_num + i]) * 8 / 1000 / 1000 / duration;

    std::cout << std::setw (5) << i+1 <<
    std::setw (12) << throughput <<
    std::setw (12) << g_signalDbmAvg[sta_num + i] <<
    std::setw (12) << g_noiseDbmAvg[sta_num + i] <<
    std::setw (12) << (g_signalDbmAvg[sta_num + i] - g_noiseDbmAvg[sta_num + i]) <<
    std::endl;
