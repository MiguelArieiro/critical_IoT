...
void MonitorSniffRx(std::string context, Ptr<const Packet> packet, uint16_t channelFreqMhz,
...
void TotalEnergy(std::string context, double oldValue, double totalEnergy)
{
  ...
  {
    NS_LOG_UNCOND(Simulator::Now().GetSeconds()
                  << "s " << nodeId << " Total energy consumed by radio = " << totalEnergy
                  << "J");
  }
...
int main(int argc, char *argv[])
{
  ...
  cmd.AddValue("numStaRxSpatialStreams", "Number of STA Rx Spatial Streams",
               numStaRxSpatialStreams);
  cmd.AddValue("numStaTxSpatialStreams", "Number of STA Tx Spatial Streams",
  ...
  cmd.AddValue("technology", "Select technology to be used. 0 = 802.11ax, 1 = 802.11n, 3 = 5G",
               technology);
  cmd.AddValue("udpPayloadSize", "UDP packet size", udpPayloadSize);
  cmd.AddValue("useExtendedBlockAck", "Enable/disable use of Extended Block Ack",
               useExtendedBlockAck);
  ...
  for (uint32_t r = 0; r < runs; r++)
  {
    ...
    for (int32_t i = 0; i < numAp; i++)
    {
      ...
      {
        staDevices[i].Add(
            wifi.Install(spectrumPhy, mac, wifiStaNodes.Get(i * numSta + j)));
      }
    ...
    for (int32_t y = 0; (y < edge_size) && (counter < numAp); y++)
    {
      for (int32_t x = 0; (x < edge_size) && (counter < numAp); x++, counter++)
      {
        ...
        mobility.SetPositionAllocator(
            "ns3::GridPositionAllocator", "MinX", DoubleValue((x - 1) * distance), "MinY",
            DoubleValue((y - 1) * distance), "DeltaX",
            DoubleValue(2 * distance / sta_edge_size), "DeltaY",
            DoubleValue(2 * distance / sta_edge_size), "GridWidth",
            UintegerValue(sta_edge_size), "LayoutType", StringValue("RowFirst"));

        if (walk)
        {
          mobility.SetMobilityModel(
              "ns3::RandomWalk2dMobilityModel", "Bounds",
              RectangleValue(Rectangle(((x - 1) * distance), ((x + 1) * distance),
                                       ((y - 1) * distance), ((y + 1) * distance))));
        }
        else
        {
    ...
    for (int32_t i = 0; i < numAp; i++)
    {
      ...
      for (int32_t j = 0; j < numSta; j++)
      {
        staticRouting = Ipv4RoutingHelper::GetRouting<Ipv4StaticRouting>(
            wifiStaNodes.Get(i * numSta + j)->GetObject<Ipv4>()->GetRoutingProtocol());
    ...
    if (useUdp)
    {
      ...
      for (int32_t i = 0; i < numAp; i++)
      {
        OnOffHelper onoff("ns3::UdpSocketFactory",
                          Address(InetSocketAddress(apInterfaces.GetAddress(i), 9)));
    ...
    else
    {
      ...
      for (int32_t i = 0; i < numAp; i++)
      {
        OnOffHelper onoff("ns3::TcpSocketFactory",
                          Address(InetSocketAddress(apInterfaces.GetAddress(i), 5000)));
    ...
    for (int32_t i = 0; i < numAp; i++)
    {
      ...
      {
        deviceModels.Add(
            radioEnergyHelper.Install(staDevices[i].Get(j), sources.Get(j + i * numSta)));
      }
    }
    ...
    if (tracing && verbose)
    {
      ...
      {
        Ptr<LiIonEnergySource> basicSourcePtr =
            DynamicCast<LiIonEnergySource>(sources.Get(i));

        basicSourcePtr->TraceConnect("RemainingEnergy", std::to_string(i),
        ...
        basicRadioModelPtr->TraceConnect("TotalEnergyConsumption", std::to_string(i),
    ...
    if (tracing)
      Config::Connect("/NodeList/*/DeviceList/*/Phy/MonitorSnifferRx",
    ...
    if (verbose)
    {
      ...
      outFile << "Flow;source;src_port;destiny;dst_port;proto;direction;tx_packets;tx_bytes;tx_"
                 "offered_raw;tx_offered_mbps;rx_bytes;rx_throughput_raw;rx_throughput_mbps;"
                 "mean_delay(ms);mean_jitter(ms);rx_packets;lost_packets;packet_loss_ratio \n";
