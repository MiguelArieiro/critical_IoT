...
int main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("channelWidth", "If set, limit testing to a specific MCS (0-11)", channelWidth);
  cmd.AddValue ("gi", "If set, limit testing to a specific MCS (0-11)", gi);
  ...
  for (int mcs = minMcs; mcs <= maxMcs; mcs+=11)
    {
      uint8_t index = 0;
      double previous = 0;
      uint8_t maxChannelWidth = frequency == 2.4 ? 40 : 160;
              ...
              basicSourceHelper.Set("LiIonEnergySourceInitialEnergyJ", DoubleValue(1000));
              // install source
              EnergySourceContainer sources = basicSourceHelper.Install(wifiStaNode);
              /* device energy model */
              ...
              {
                  deviceModels.Add(radioEnergyHelper.Install(staDevice, sources.Get(i)));
              }
              ...
              for (DeviceEnergyModelContainer::Iterator iter = deviceModels.Begin (); iter != deviceModels.End (); iter ++)
                {
                  ...
                  NS_LOG_UNCOND (energyConsumed << "J");
                }
